...
var comMsg = function(msg) {
	return GetLen(msg) >= 38 ? "<font class='f_1'>" + msg + "</font>"
			: "<font class='f_0'>" + msg + "</font>";
}
	...
	if (keyCode == 40 || keyCode == 38 || keyCode == 13) {
		...
		if (keyCode == 40) {
			...
			if (tipindex == vschool.find("span").length) {
				tipindex = 0;
				vschool.find("span").eq(vschool.find("span").length - 1).css(
						"background-color", "");
			}
	...
	if (mlogoNum == 1) {
		...
		for (var i = 0; i < tsmail_Arr.length; i++) {
			if (tsmail_Arr[i].indexOf(rightMstr) == 0 && rightMstr) {
				...
				mailist += '<span onmousedown="email2val(\'userMam\',this);">'
						+ mail + '</span>';
			}
			if (rightMstr == '') {
...
var ckmem = function(isCk) {
	...
	if (!isCk && !val) {
		 ...
		 * if(val==''){ showError(MailMId,MailMErrId,memArr[0]);
		 * $("#person_mail").hide().html(''); return false; }
		 */
	if (!mobile_preg(val)) {
	...
	if (num_preg.test(val)) {
		...
		var bol = false;
		$("#" + MailMErrId).attr("class", "").addClass("reg_error1").html(
				comMsg("检测中...."));
		$
				.post(
						"/ajax/verifmobile/",
						...
						function(data) {
							if (data != "y") {
								if (data.indexOf("请直接登录") >= 0) {
									var newData = comMsg(data)
											.replace("请直接登录",
													"<a href='/' style='color:#669900'>请直接登录</a>");
									...
									$("#" + MailMId).parent().attr("class", "")
											.addClass("regM errorBorder")
											.next().attr("class", "");
									$("#" + MailMErrId).attr("class",
											"reg_error").html(newData);
								} else {
	...
	/*
	 * else { mb=0; showcodeDiv(2); if( !mail_preg(val) ){
	 * showError(MailMId,MailMErrId,memArr[2]);
	 * $("#person_mail").hide().html(''); return false; }
	 * $("#person_mail").hide().html(''); $.ajaxSetup({async:false });
	 * $("#"+MailMErrId).attr("class","").addClass("reg_error1").html(comMsg("检测中...."));
	 * var bol=false;
	 * $.post("/ajax/verifemails/",{"name":"outemail","param":val},function(data){
	 * if(data!="y"){ if(data.indexOf("请直接登录")>=0){ var newData =
	 * comMsg(data).replace("请直接登录","<a href='/' style='color:#669900'>请直接登录</a>");
	 * if(data.indexOf("宜立方速运")>-1){ newData = "该邮箱在宜立方速运已注册，请<a href='/'
	 * style='color:#669900'>直接登录></a>或<a
	 * href='/reg/findpass/?returnUrl=http://www.e3mall.cn/' target='_blank'
	 * style='color:#669900'>找回密码></a>"; }
	 * $("#"+MailMId).parent().attr("class","").addClass("regM
	 * errorBorder").next().attr("class","");
	 * $("#"+MailMErrId).attr("class","reg_error").html(newData); }else{
	 * showError(MailMId,MailMErrId,data); } return false; }
	 * $("#nameRegOk").val(1); showSuccess(MailMId,MailMErrId,OkArr[0]);
	 * bol=true; }); return bol; }
	 */
}
// 密码强度显示
...
var ckpwd = function(isCk, keyup) {
	...
	if (val == "") {
		...
		$("#" + PwdId).parent().attr("class", "")
				.addClass("regM defaultBorder");
...
var ckmcode = function(isCk, keyup) {
	...
	if (!isCk && !mcode) {
		$("#" + MobileCodeId).attr("class", "").addClass(
				"regCodeInput defaultBorder");
		$("#" + MobileCodeId).next().attr("class", "");
	...
	$("#" + MobileCodeErrId).attr("class", "").addClass("reg_error1").html(
			comMsg("检测中...."));
	var bol = false;
	$.post("/ajax/VerifSmscode/", {
		"mobile" : mobileNum,
		"param" : mcode
	}, function(data) {
		if (data != 'y') {
		...
		$("#" + MobileCodeId).attr("class", "regCodeInput defaultBorder")
				.next().attr("class", "regOk1").css("left", "246px");
...
var ckcode = function(isCk, keyup) {
	...
	if (Authcode.length < 4 && keyup == 1) {
		$("#" + pageCodeId).attr("class", "").addClass(
				"regCodeInput defaultBorder").next().attr("class", "");
	...
	$("#" + pageCodeErrId).attr("class", "").addClass("reg_error1").html(
			comMsg("检测中...."));
	var bol = false;
	$.post("/reg/codes/", {
		"name" : "auth_code",
		...
		if (data != 'y') {
			...
			$("#" + pageCodeId).attr("class", "").addClass(
					"regCodeInput defaultBorder").next()
					.attr("class", "regOk1");
...
var getmcode = function() {
	...
	if (!a1) {
		$.post("/other/getsms/", {
			...
			if (data == 'y') {
				ctDown('send');
				$("#" + MobileCodeErrId).attr("class", "reg_error1").html(
						comMsg("短信验证码已发送，请查收。"));
				return;
			}
		});
...
var ckdmcode = function(isCk, keyup) {
	...
	if (dmNum.length < 6 && keyup == 1) {
		$("#" + sfCodeId).attr("class",
				"regInvitationInput defaultBorder floatleft").next().attr(
				"class", "");
	...
	if (!isCk && !dmNum) {
		$("#" + sfCodeId).attr("class",
				"regInvitationInput defaultBorder floatleft");
		$("#" + sfCodeErrId).attr("class", "").html("");
	}
	if (dmNum) {
		if (dmNum.length != 6 && dmNum.length != 12 && dmNum.length != 14) {
			$("#" + sfCodeId).attr("class",
					"regInvitationInput errorBorder floatleft");
			$("#" + sfCodeErrId).attr("class", "reg_error").html(
					comMsg("邀请码错误"));
			return false;
		}
		$.ajaxSetup({
		...
		$("#" + sfCodeErrId).attr("class", "").addClass("reg_error1").html(
				comMsg("检测中...."));
		var bol = false;
		$.post("/ajax/validatedm/", {
			"name" : sfCodeId,
			"param" : dmNum
		}, function(data) {
			if (data != "y") {
				...
				$("#" + sfCodeId).next().attr("class", "regOk1").css("left",
						"243px");
				$("#" + sfCodeId).attr("class",
						"regInvitationInput defaultBorder floatleft");
...
var ckAgree = function() {
	...
	if (Agreestat == false) {
		$("#" + AgreementErrId).attr("class", "").addClass("reg_error").html(
				comMsg(agreeArr[0]));
		return false;
	}
...
var sub_data = function() {
	...
	if (!a1 || a2 == false || a3 == false || a4 == false || a5 == false
			|| a6 == false) {
		return false;
	}
	$("#sub_per").html(
			'<a class="unit_login_in" href="javascript:void(0);">注册中...</a>');
	$
			.post(
					"/register/Regperson/",
					$('#regForm_mod').serialize().replace(/\+/g, "")
							+ "&tjuid=" + tjuid,
					function(data) {
						if (data == "y") {
							// 跳转到注册成功页面
						...
						$("#sub_per")
								.html(
										'<a href="javascript:void(0);" class="registerNow" id="reg_per_data">立即注册</a>');
...
var ckcpyname = function(isCk) {
	...
	var bol = false;
	$.post("/ajax/verifUsername/", {
		"name" : "userName",
		"param" : val
	}, function(data) {
		if (data != "y") {
			if (data.indexOf("请直接登录") >= 0) {
				$("#" + cpyUserNameId).parent().attr("class",
						"regM errorBorder").next().attr("class", "");
				var msg_new = comMsg(data).replace("请直接登录",
						"<a href='/' style='color:#669900'>请直接登录</a>");
				$("#" + cpyUserNameErrId).attr("class", "reg_error3 f-color")
						.html(msg_new);
			} else {
	...
	$
			.post(
					...
					function(data) {
						if (data != "y") {
							if (data.indexOf("请直接登录") >= 0) {
								$("#" + cpyMobileId).parent().attr("class",
										"regM errorBorder").next().attr(
										"class", "");
								var msg_new = comMsg(data)
										.replace("请直接登录",
												"<a href='/' style='color:#669900'>请直接登录</a>");
								if (data.indexOf("宜立方速运") > -1) {
								...
								$("#" + cpyMobileErrId).attr("class",
										"reg_error3 f-color").html(msg_new);
							} else {
...
var ckcpycode = function(isCk, keyup) {
	...
	if (Authcode.length < 4 && keyup == 1) {
		$("#" + cpyCodeId).attr("class", "regCodeInput defaultBorder").next()
				.attr("class", "");
...
var ckcpy_xy = function() {
	...
	if (val == false) {
		$("#" + cpyAgreeErrId).attr("class", "reg_error3 f-color").html(
				comMsg(cpyAgreeArr[0]));
		return false;
	}
...
var cpy_subdata = function() {
	...
	if (cm1 || cm2 || cm3 || cm4 || cm5 || cm6 || cm7 || cm8 || cm9 == 0
			|| cm10 == 0 || cm11 || cm12 || cm13 || cm14 || cm15) {
		return false;
	}

	$.post("/reg/Regcompany/",
			$('#regForm_cpy').serialize().replace(/\+/g, ""), function(data) {
				if (data == "y") {
					// 跳转到注册成功页面
